...
Usage:
    py -3 task.py create "<title>" [--slug <name>] [--assignee <dev>] [--priority P0|P1|P2|P3] [--parent <dir>] [--package <pkg>] [--no-start]
    py -3 task.py add-context <dir> <file> <path> [reason] # Add jsonl entry
    py -3 task.py validate <dir>              # Validate jsonl files
    py -3 task.py list-context <dir>          # List jsonl entries
    py -3 task.py start <dir>                 # Set active task
    py -3 task.py current [--source]          # Show active task
    py -3 task.py finish                      # Clear active task
    py -3 task.py set-branch <dir> <branch>   # Set git branch
    py -3 task.py set-base-branch <dir> <branch>  # Set PR target branch
    py -3 task.py set-scope <dir> <scope>     # Set scope for PR title
    py -3 task.py archive <task-dir>          # Archive completed task
    py -3 task.py list                        # List active tasks
    py -3 task.py list-archive [month]        # List archived tasks
    py -3 task.py add-subtask <parent-dir> <child-dir>     # Link child to parent
    py -3 task.py remove-subtask <parent-dir> <child-dir>  # Unlink child from parent
"""
...
Usage:
  py -3 task.py create <title>                     Create new task directory
  py -3 task.py create <title> --package <pkg>     Create task for a specific package
  py -3 task.py create <title> --parent <dir>      Create task as child of parent
  py -3 task.py create <title> --no-start          Create without making it active in this session
  py -3 task.py add-context <dir> <jsonl> <path> [reason]  Add entry to jsonl
  py -3 task.py validate <dir>                     Validate jsonl files
  py -3 task.py list-context <dir>                 List jsonl entries
  py -3 task.py start <dir>                        Set active task
  py -3 task.py current [--source]                 Show active task
  ...
  py -3 task.py set-scope <dir> <scope>            Set scope for PR title
  py -3 task.py archive <task-dir>                 Archive completed task
  py -3 task.py add-subtask <parent> <child>       Link child task to parent
  py -3 task.py remove-subtask <parent> <child>    Unlink child from parent
  py -3 task.py list [--mine] [--status <status>]  List tasks
  py -3 task.py list-archive [YYYY-MM]             List archived tasks
...
Examples:
  py -3 task.py create "Add login feature" --slug add-login
  py -3 task.py create "Add login feature" --slug add-login --package cli
  py -3 task.py create "Child task" --slug child --parent .trellis/tasks/01-21-parent
  py -3 task.py add-context <dir> implement .trellis/spec/cli/backend/auth.md "Auth guidelines"
  py -3 task.py set-branch <dir> task/add-login
  py -3 task.py start .trellis/tasks/01-21-add-login
  py -3 task.py current --source
  py -3 task.py finish
  py -3 task.py archive add-login
  py -3 task.py add-subtask parent-task child-task  # Link existing tasks
  py -3 task.py remove-subtask parent-task child-task
  py -3 task.py list                               # List all active tasks
  py -3 task.py list --mine                        # List my tasks only
  py -3 task.py list --mine --status in_progress   # List my in-progress tasks
""")
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print(
            "  py -3 ./.trellis/scripts/get_context.py --mode phase --step 1",
            file=sys.stderr,
        )
        print(
