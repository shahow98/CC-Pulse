...
```bash
py -3 ./.trellis/scripts/get_context.py --mode packages   # list packages / layers
```
...
py -3 ./.trellis/scripts/task.py start <name>          # set active task (session-scoped when available)
py -3 ./.trellis/scripts/task.py current --source      # show active task and source
py -3 ./.trellis/scripts/task.py finish                # clear active task (triggers after_finish hooks)
py -3 ./.trellis/scripts/task.py archive <name>        # move to archive/{year-month}/
...
py -3 ./.trellis/scripts/task.py set-base-branch <name> <branch>    # PR target
...
> Run `py -3 ./.trellis/scripts/task.py --help` to see the authoritative, up-to-date list.
...
```bash
py -3 ./.trellis/scripts/get_context.py                            # full session runtime
py -3 ./.trellis/scripts/get_context.py --mode packages            # available packages + spec layers
py -3 ./.trellis/scripts/get_context.py --mode phase --step <X.Y>  # detailed guide for a workflow step
```
...
# e.g. py -3 ./.trellis/scripts/get_context.py --mode phase --step 1.1
```
...
Skip when `py -3 ./.trellis/scripts/task.py current --source` already points to a task.
...
**Final pass (before Phase 3.4 commit)**: the last 2.2 of a task must run full-scope, not just on the latest implement chunk. List all affected packages with `py -3 ./.trellis/scripts/get_context.py --mode packages`, then load each package's spec index Quality Check section. This catches cross-layer / multi-package issues a mid-iteration local 2.2 cannot.
