...
public static class HookConfigurator
{
    ...
    private static readonly (string Matcher, string Suffix)[] SessionStartHooks =
        [("startup", "start"), ("clear", "start"), ("compact", "start")];
    ...
    public static int Configure(string hookExePath)
    {
        try
        {
            // Convert hook path to forward slashes for Git Bash compatibility
            var hookPath = hookExePath.Replace('\\', '/').TrimEnd('/');
            ...
            foreach (var (eventName, entries) in HooksConfig)
            {
                ...
                {
                    var ccPulseHook = new JsonObject
                    {
                        ["type"] = "command",
                    ...
                    if (existingEntry is not null)
                    {
                        ...
                        if (existingHooks is not null)
                        {
                            ...
                            {
                                if (h?["command"]?.GetValue<string>()?.Contains("cc-pulse-hook") == true ||
                                    h?["command"]?.GetValue<string>()?.Contains("ClaudeMonitor") == true)
                                {
                                    alreadyExists = true;
                                    break;
                                }
