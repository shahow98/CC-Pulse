...
def _build_compact_current_state(
    ...
    if get_tasks_dir and iter_active_tasks:
        try:
            ...
            lines.append(
                f"Active tasks: {task_count} total. Use `py -3 ./.trellis/scripts/task.py list --mine` only if needed."
            )
        except Exception:
...
def _build_workflow_overview(workflow_path: Path) -> str:
    ...
    out_lines = [
        ...
        "Full guide: .trellis/workflow.md. Step detail: `py -3 ./.trellis/scripts/get_context.py --mode phase --step <X.Y>`.",
        "",
    ]
